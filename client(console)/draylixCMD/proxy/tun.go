...
func (h HttpTunnel) StartProxy() (in, out int64) {
	g := sync.WaitGroup{}
	g.Add(2)
	go transmit(h.Local, h.Remote, &g, &in)
	go transmit(h.Remote, h.Local, &g, &out)
	g.Wait()
	return
}
...
func transmit(dst, src net.Conn, g *sync.WaitGroup, flow *int64) {
...
func (s *Socks5Tunnel) StartProxy() (in, out int64) {
	g := sync.WaitGroup{}
	g.Add(2)
	go transmit(s.Local, s.Remote, &g, &in)
	go transmit(s.Remote, s.Local, &g, &out)
	g.Wait()
	return
}
